/** AI 记账对话页面 — SSE 流式渲染 + 图片 OCR + 二次确认（所有状态原地更新） */
...
export default function ChatPage() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>(() => {
    try { const saved = localStorage.getItem('billagent_messages'); return saved ? JSON.parse(saved) : []; }
    catch { return []; }
  });
  ...
  const [statusText, setStatusText] = useState('');       // 所有状态原地更新
  ...
  const handleConfirm = useCallback((confirmData: ConfirmRequired, modifiedArgs?: Record<string, unknown>) => {
    ...
    streamCtrlRef.current = chatApi.confirmAction(
      { session_id: sessionId, action: 'confirm', modified_arguments: modifiedArgs },
      {
          ...
          switch (evt.event) {
            ...
            case 'tool_call': {
              try {
                ...
                setStatusText(TOOL_LABELS[tc.tool_name] || `正在执行 ${tc.tool_name}...`);
              } catch { /* */ }
              break;
            }
        ...
        onError: (err) => { appendAssistant(`\n\n❌ 出错了: ${err.message}`); setStatusText(''); setLoading(false); },
        onDone: (sid) => { if (sid) setSessionId(sid); setStatusText(''); setLoading(false); },
      }
    );
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      addMsg({ role: 'assistant', content: `❌ OCR 识别失败: ${err instanceof Error ? err.message : '未知错误'}`, timestamp: Date.now() });
  ...
  const renderConfirmCard = (
    ...
    const confirmWithEdit = () => {
      ...
      setEditingCardId(null); setEditForm({});
    };
    ...
    if (msg.confirmed) {
      return (
        <div className="flex items-center gap-2 my-1 px-4 py-2 bg-green-50 border border-green-200 rounded-xl text-sm">
          <span>✅</span><span className="text-green-700 font-medium">已记账</span>
          <span className="text-green-500 text-xs ml-auto">{String(args.payee || '')} {amount != null ? `${Math.abs(amount).toFixed(0)}元` : ''}</span>
        </div>
      );
    }
    if (msg.rejected) {
      return (
        <div className="flex items-center gap-2 my-1 px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm">
          <span>❌</span><span className="text-gray-500">已取消</span>
        </div>
      );
    }
    ...
    // 待确认
    return (
      <div className="my-2 px-4 py-3 bg-amber-50/80 border border-amber-200 rounded-xl shadow-sm">
        <div className="flex items-center gap-2 mb-3">
          <span className="text-lg">⚠️</span>
          <span className="font-semibold text-amber-800 text-sm">确认记账？</span>
          <span className="text-xs text-amber-500 ml-auto">{isEditing ? '✏️ 修改中' : 'AI 将创建此账单'}</span>
        </div>

        <div className="bg-white rounded-lg p-3 mb-3 space-y-1.5 text-sm border border-amber-100">
          {isEditing ? (
            <>
              <EditField label="金额" value={editForm.amount} onChange={(v) => updateField('amount', v)} type="number" hint="支出为负数" />
              ...
              <EditField label="商户" value={editForm.payee} onChange={(v) => updateField('payee', v)} />
              <EditField label="描述" value={editForm.description} onChange={(v) => updateField('description', v)} />
              <EditField label="日期" value={editForm.transaction_date} onChange={(v) => updateField('transaction_date', v)} type="date" />
              <EditField label="支付方式" value={editForm.payment_method} onChange={(v) => updateField('payment_method', v)} />
            </>
          ) : (
            <>
              <DetailRow label="金额" value={amount != null ? `${isExpense ? '-' : '+'}${Math.abs(amount).toFixed(2)}元` : '未知'} emphasis />
              {displayArgs.category ? <DetailRow label="分类" value={String(displayArgs.category)} /> : null}
              {displayArgs.payee ? <DetailRow label="商户" value={String(displayArgs.payee)} /> : null}
              {displayArgs.description ? <DetailRow label="描述" value={String(displayArgs.description)} /> : null}
              {displayArgs.transaction_date ? <DetailRow label="日期" value={String(displayArgs.transaction_date)} /> : null}
              {displayArgs.payment_method ? <DetailRow label="支付方式" value={String(displayArgs.payment_method)} /> : null}
            </>
          )}
        </div>
        ...
        <div className="flex gap-2">
          {isEditing ? (
            <>
              <button onClick={confirmWithEdit} className="flex-1 px-4 py-2 bg-green-500 text-white rounded-lg text-sm font-medium hover:bg-green-600 transition-colors">✅ 确认修改并记账</button>
              <button onClick={cancelEdit} className="flex-1 px-4 py-2 bg-white text-gray-500 border border-gray-300 rounded-lg text-sm hover:bg-gray-50 transition-colors">取消修改</button>
            </>
          ) : (
            <>
              <button onClick={() => onConfirm(msg.confirmData!)} className="flex-1 px-4 py-2 bg-green-500 text-white rounded-lg text-sm font-medium hover:bg-green-600 transition-colors shadow-sm">✅ 确认记账</button>
              <button onClick={startEdit} className="px-4 py-2 bg-white text-blue-500 border border-blue-300 rounded-lg text-sm hover:bg-blue-50 transition-colors">✏️ 修改</button>
              <button onClick={() => onReject(msg.confirmData!)} className="px-4 py-2 bg-white text-gray-500 border border-gray-300 rounded-lg text-sm hover:bg-gray-50 transition-colors">取消</button>
            </>
          )}
        </div>
      </div>
    ...
    return (
      <div className="mx-4 my-3 p-4 bg-green-50/80 border border-green-200 rounded-xl shadow-sm">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="font-semibold text-green-800 text-sm">OCR 识别完成 · {ocrResult.items.length} 条记录 · 置信度 {ocrResult.confidence}</span>
        </div>
        <div className="space-y-2">
          {ocrResult.items.map((item, i) => (
            <div key={i} className="flex flex-wrap gap-x-4 gap-y-1 text-sm bg-white rounded-lg p-3 shadow-sm">
              ...
              <span className={item.amount && item.amount < 0 ? 'text-rose-600' : 'text-emerald-600'}>{item.amount != null ? `${item.amount}元` : ''}</span>
      ...
      <header className="flex items-center justify-between px-6 py-4 bg-white/90 backdrop-blur border-b shrink-0">
        ...
        <div className="flex items-center gap-3">
          <label className="flex items-center gap-1.5 text-xs cursor-pointer select-none" title="开启后，AI 创建账单前需要你确认">
            <input type="checkbox" checked={confirmMode} onChange={(e) => setConfirmMode(e.target.checked)} className="w-3.5 h-3.5 rounded accent-primary-500" />
          ...
          <button onClick={() => { setMessages([]); setSessionId(null); setOcrResult(null); setStatusText(''); localStorage.removeItem('billagent_messages'); localStorage.removeItem('billagent_session_id'); }}
            className="text-sm text-gray-400 hover:text-gray-600 transition-colors">新对话</button>
        </div>
      </header>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto py-4 space-y-1">
        ...
        {messages.map((msg, i) => {
          if (msg.role === 'confirm_card') {
            return <div key={i} className="px-4 py-1">{renderConfirmCard(msg, handleConfirm, handleReject)}</div>;
          }
          // 不再渲染 tool_status 消息 — 所有工具状态统一通过 statusText 原地展示
          if (msg.role === 'tool_status') return null;
          const isUser = msg.role === 'user';
          if (!isUser && !msg.content) return null; // 空 assistant 占位不渲染

          return (
            <div key={i} className={`px-4 py-1 ${isUser ? '' : 'bg-gray-50/50'}`}>
              <div className={`flex gap-3 ${isUser ? 'flex-row-reverse' : ''}`}>
                <span className="text-2xl shrink-0 mt-1">{isUser ? '👤' : '🤖'}</span>
                <div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm leading-relaxed shadow-sm ${isUser ? 'bg-primary-500 text-white' : 'bg-white border border-gray-200 text-gray-800'}`}>
                  {isUser ? (
                    <p className="whitespace-pre-wrap">{msg.content}</p>
                  ) : (
                    <ReactMarkdown components={{ p: ({ children }) => <p className="mb-1 last:mb-0">{children}</p>, code: ({ children }) => <code className="bg-gray-100 text-red-600 px-1 py-0.5 rounded text-xs">{children}</code> }}>
                      {msg.content || '...'}
                    </ReactMarkdown>
                  )}
                </div>
      ...
      <div className="p-4 bg-white/90 backdrop-blur border-t shrink-0">
        <div className="flex items-end gap-2">
          <button onClick={() => fileInputRef.current?.click()} disabled={ocrLoading}
            className="shrink-0 p-3 text-gray-400 hover:text-primary-500 hover:bg-primary-50 rounded-xl transition-colors disabled:opacity-50" title="上传账单截图">📎</button>
          ...
          <textarea value={input} onChange={(e) => setInput(e.target.value)}
            ...
            placeholder="说说你的账单... (Enter 发送, Shift+Enter 换行)" rows={1}
            className="flex-1 resize-none rounded-xl border border-gray-200 px-4 py-3 text-sm focus:outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500 placeholder:text-gray-300"
            disabled={loading} />
          <button onClick={handleSend} disabled={loading || !input.trim()}
            className="shrink-0 px-5 py-3 bg-primary-500 text-white rounded-xl font-medium hover:bg-primary-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm">发送</button>
        </div>
        <p className="text-xs text-gray-400 mt-2 text-center">AI 可能产生不准确信息，请核对记账结果</p>
      </div>
    </div>
  );
}
...
function DetailRow({ label, value, emphasis }: { label: string; value: string; emphasis?: boolean }) {
  return (
    <div className="flex justify-between">
      ...
      <span className={emphasis ? 'font-bold text-gray-800' : 'text-gray-700'}>{value}</span>
    </div>
  );
}
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <input type={type} value={value} onChange={(e) => onChange(e.target.value)}
        className="flex-1 border rounded px-2 py-1 text-sm focus:outline-none focus:border-primary-500" placeholder={hint} />
    </div>
  );
}
