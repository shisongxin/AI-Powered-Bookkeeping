...
const BudgetPage: React.FC = () => {
  ...
  const handleApplySuggestion = async (category: string, amount: number) => {
    try {
      ...
      Taro.showToast({ title: err?.message || '应用失败', icon: 'none' })
    }
  }

  const items = (budgetVs?.items || []).sort((a: any, b: any) => b.budget - a.budget)
  const totalBudget = budgetVs?.total_budget || 0
  const totalActual = budgetVs?.total_actual || 0
  const totalRemaining = budgetVs?.total_remaining || 0
  const hasBudget = items.length > 0

  return (
    <ScrollView className='budget-container' scrollY enableFlex>
      {/* ===== 深色头部区域 ===== */}
      <View className='budget-header'>
        <View className='header-title-row'>
          ...
          <Text className='header-title'>预算管理</Text>
        ...
        <View className='budget-summary'>
          <View className='summary-item'>
            <Text className='summary-label'>预算合计</Text>
            <Text className='summary-value'>¥{totalBudget.toFixed(0)}</Text>
          </View>
          <View className='summary-item'>
            <Text className='summary-label'>实际支出</Text>
            <Text className='summary-value expense'>¥{Math.abs(totalActual).toFixed(0)}</Text>
          </View>
          <View className='summary-item'>
            <Text className='summary-label'>剩余</Text>
            <Text className={`summary-value ${totalRemaining >= 0 ? 'remaining-positive' : 'remaining-negative'}`}>
      ...
      <View className='budget-actions'>
        <View className='budget-action-btn gold' onClick={handleAutoGenerate}>
          <Text className='action-text'>{autoGenLoading ? '生成中...' : '⚡ 智能生成'}</Text>
        </View>
        <View className='budget-action-btn' onClick={handleSuggest}>
          <Text className='action-text'>{suggestLoading ? '分析中...' : '🤖 AI 建议'}</Text>
        </View>
        <View className={`budget-action-btn ${settingBudget ? 'active' : ''}`} onClick={() => setSettingBudget(!settingBudget)}>
          <Text className='action-text'>{settingBudget ? '✓ 完成' : '✏️ 手动设置'}</Text>
        </View>
      </View>
          ...
          {suggestions.map((s: any) => (
            <View key={s.category} className='suggestion-item'>
              <View className='suggestion-info'>
                ...
                <Text className='suggestion-reason' numberOfLines={2}>{s.reason}</Text>
              </View>
              <View className='suggestion-right'>
                <Text className='suggestion-amount'>¥{(s.suggested_amount || 0).toFixed(0)}</Text>
      ...
      {/* ===== 加载中 ===== */}
      {loading && <View className='skeleton-card' />}

      {/* ===== 预算执行列表 ===== */}
      {!loading && hasBudget && (
        <View className='budget-section'>
          <Text className='section-title'>预算执行</Text>
          <Text className='section-subtitle'>{items.length} 个分类 · {year}年{month}月</Text>
          {items.map((item: any) => {
            const pct = Math.min(item.percentage, 100)
            const barColor = item.status === '已超支' ? '#ef4444' : item.status === '接近上限' ? '#f59e0b' : '#10b981'
            const badgeClass = item.status === '已超支' ? 'red' : item.status === '接近上限' ? 'gold' : 'green'
            ...
            return (
              <View key={item.category} className='budget-item'>
                <View className='budget-item-top'>
                  ...
                  <View className={`budget-badge ${badgeClass}`}>
                ...
                <View className='budget-item-bottom'>
                  <Text className='budget-amount-actual'>实际 ¥{Math.abs(item.actual).toFixed(0)}</Text>
                  <Text>预算 ¥{item.budget.toFixed(0)}</Text>
                </View>
                {settingBudget && (
                  <View className='budget-edit-row'>
                    <Input className='budget-input' type='digit' placeholder='新预算金额'
      ...
      {/* ===== 空状态 ===== */}
      {!loading && !hasBudget && (
        <View className='budget-section budget-empty'>
          <Text className='empty-icon'>💰</Text>
          <Text className='empty-text'>本月未设置预算</Text>
          <Text className='empty-hint'>点击上方按钮智能生成或手动设置</Text>
        </View>
      )}

      {/* ===== 快速设置（未设置预算的分类） ===== */}
      {settingBudget && categories.length > 0 && (
        <View className='budget-section'>
          <Text className='section-title'>快速设置</Text>
          <Text className='section-subtitle'>点击分类图标快速设置预算</Text>
